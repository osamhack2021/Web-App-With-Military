...
export default function Board({
  ...
  const remove = (board_info) => {
    dispatch(removeBoard({ boardId: board_info._id })).then((response) => {
    ...
    <Box sx={{ py: 3, borderBottom: "1px solid rgba(0, 0, 0, 0.3)" }}>
      <Box sx={{ display: "flex" }}>
        ...
        {loginData._id !== undefined && (
            ...
            <IconButton
              onClick={(e) => {
                removeBoardOnConfirm(e, boardInfo);
              }}
            >
              <CloseOutlinedIcon />
            </IconButton>
      ...
      <Box sx={{ display: "flex" }}>
        ...
        {parseInt(timeDiff / 3600 / 24 / 365) ? (
          <Typography>{parseInt(timeDiff / 3600 / 24 / 365)}년 전</Typography>
        ) : parseInt(timeDiff / 3600 / 24 / 30) ? (
          <Typography>{parseInt(timeDiff / 3600 / 24 / 30)}달 전</Typography>
        ) : parseInt(timeDiff / 3600 / 24) ? (
          <Typography>{parseInt(timeDiff / 3600 / 24)}일 전</Typography>
        ) : parseInt(timeDiff / 3600) ? (
          <Typography>{parseInt(timeDiff / 3600)}시간 전</Typography>
        ) : parseInt((timeDiff % 3600) / 60) ? (
          <Typography>{parseInt((timeDiff % 3600) / 60)}분 전</Typography>
        ) : (
      ...
      <Comment boardInfo={boardInfo} refreshComment={refreshComment} />

      {editMode && (
        <EditBoard boardInfo={boardInfo} toggleEditMode={toggleEditMode} />
      )}
    </Box>
  );
}
